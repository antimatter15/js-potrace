...
class Potrace
{
    ...
    double COS179 = Math.Cos(179 * Math.PI / 180);
    /// <summary>
    ...
    struct Curve
    {
      ...
      /// <param name="B">Endpoint</param>
        Curve(CurveKind Kind, dPoint A, dPoint ControlPointA, dPoint ControlPointB, dPoint B)
        {
    ...
    private class privcurve
    {
        ...
        dPoint[,] ControlPoints;/* c[n][i]: control points. 
    ...
    dPoint bezier(double t, dPoint p0, dPoint p1, dPoint p2, dPoint p3)
    {
        double s = 1 - t;
        dPoint res;
    ...
    /* calculate the point t in [0..1] on the (convex) bezier curve
        ...
        solution in [0..1]. */
    double tangent(dPoint p0, dPoint p1, dPoint p2, dPoint p3, dPoint q0, dPoint q1)
    {
    ...
    double ddist(dPoint p, dPoint q)
    {
        return Math.Sqrt((p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y));
    }
    /* calculate p1 x p2 */
    int xprod(iPoint p1, iPoint p2)
    {
        return p1.x * p2.y - p1.y * p2.x;
    }
    /* calculate p1 x p2 */
    double xprod(dPoint p1, dPoint p2)
    {
        return p1.x * p2.y - p1.y * p2.x;
    }
    /* calculate (p1-p0)x(p3-p2) */
    double cprod(dPoint p0, dPoint p1, dPoint p2, dPoint p3)
    {
        double x1, y1, x2, y2;
    ...
    double iprod(dPoint p0, dPoint p1, dPoint p2)
    {
        double x1, y1, x2, y2;
    ...
    double iprod1(dPoint p0, dPoint p1, dPoint p2, dPoint p3)
    {
        double x1, y1, x2, y2;
    ...
    iPoint dorth_infty(dPoint p0, dPoint p2)
    {
        iPoint r;
    ...
    dPoint interval(double lambda, dPoint a, dPoint b)
    {
        dPoint res;
    ...
    double dpara(dPoint p0, dPoint p1, dPoint p2)
    {
        double x1, y1, x2, y2;
    ...
    double ddenom(dPoint p0, dPoint p2)
    {
    ...
    void pointslope(Path pp, int i, int j, ref dPoint ctr, ref dPoint dir)
    {
        /* assume i<j */
    ...
    class MonotonInterval
    {
        ...
        int from;
        int to;
        void ResetCurrentID(int modulo)
        {
            if (!Increasing)
        ...
        MonotonInterval(bool Increasing, int from, int to)
        {
    ...
    bool[,] BitMapToBinary(Bitmap b, int _treshold)
    {
    ...
    Bitmap BinaryToBitmap(bool[,] Matrix, bool IgnoreBorder)
    {
    ...
    private bool FindNext(bool[,] Matrix, ref int x, ref int y)
    {
    ...
    /// <param name="x">x index in the source Matrix</param>
    /// <param name="y">y index in the source Matrix</param>
    bool FindNext(bool[,] Matrix, ref int x, ref int y, Path P)
    {
        int i = 0;
        int n = P.pt.Length;
    ...
    double quadform(double[,] Q, dPoint w)
    {
        double[] v ={ w.x, w.y, 1 };
        int i, j;
        double sum = 0;
    ...
    /// <returns></returns>
    Path findpath(bool[,] Matrix, iPoint Start)
    {
    ...
    void findNextTrace(bool[,] Matrix, ref int x, ref int y, ref direction Dir)
    {
        switch (Dir)
        {
            case direction.West:
    ...
    ArrayList GetMonotonIntervals(iPoint[] Pts)
    {
    ...
    void Xor_Path(bool[,] Matrix, Path P)
    {
        int i = 0;
        int n = P.pt.Length;
        ArrayList MonotonIntervals = P.MonotonIntervals;
    ...
    double penalty3(Path pp, int i, int j)
    {
        int n = pp.pt.Length;
...
such that all j with i<j<k lie on a line connecting i,k. */
 ...
 segment */
    ...
    void smooth(privcurve curve, int sign, double alphamax)
    {
        int m = curve.n;
    ...
    bool opti_penalty(Path pp, int i, int j, ref opti res, double opttolerance, int[] convc, double[] areac)
    {
        int m = pp.Curves.n;
        int k, k1, k2, conv, i1;
...
on failure. */
    void opticurve(Path pp, double opttolerance)
    {
        int m = pp.Curves.n;
    ...
    void getContur(bool[,] bm, int x, int y, ArrayList plistp)
    {
    ...
    void potrace_trace(bool[,] bm, ArrayList ListOfCurveArrays)
    {
    ...
    void AddCurve(ArrayList Curves, dPoint A, dPoint ControlPointA, dPoint ControlPointB, dPoint B)
    {
    ...
    void PathList_to_ListOfCurveArrays(ArrayList plistp, ArrayList ListOfCurveArrays)
    {
        ArrayList plist;
        ...
        for (int j = 0; j < plistp.Count; j++)
        {
            ...
            for (int i = 0; i < plist.Count; i++)
            {
                ...
                if (Curves.Count > 0)
                {
                    Curve CL = Curves[Curves.Count - 1];
                    Curve CF = Curves[0];
                    ...
                    for (int ci = 0; ci < Curves.Count; ci++) CList[ci] = Curves[ci];
                    clist.Add(CList);
                }
            }
    ...
    void bm_to_pathlist(bool[,] bm, ArrayList plistp)
    {
        int x = 0, y = 0;
    ...
    void process_path(ArrayList plistp)
    {
        Path p;
        ArrayList plist;
        /* call downstream function with each path */
